...
``` r
mass2=vector("numeric",length = length(dinosaur_lengths))

for (i in seq_along(dinosaur_lengths)) {
  
mass2[i]= a_values[i]*dinosaur_lengths[i]^ b_values[i]
...
Answer: **They are all named in the format buoy_year, they come from the
same repo, and they all have the same na values.The only difference is
the year for each file.**
...
for (year in start:end){
  path <- str_c("https://raw.githubusercontent.com/nt246/NTRES-6100-data-science/master/datasets/buoydata/44013_", year, ".csv")
  df<- read_csv(path,na = c("99", "999", "99.00", "999.0")) |> 
  rename("Year"=YY,"Month"=MM,"wave_height"=WVHT,"temp"=WTMP) |> 
